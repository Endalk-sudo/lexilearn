      ...
      <div className="grid gap-4 xl:grid-cols-[230px_minmax(0,1fr)_330px]">
          ...
          <CardContent className="space-y-2 pt-0">
            {overview.branches.map((b)=>{
              ...
              return <button key={b.id} onClick={()=>setBranchId(b.id)} className={cn('w-full rounded-xl border p-3 text-left transition-all',b.id===branchId?'border-primary/40 bg-primary/5 shadow-sm':'hover:border-primary/20 hover:bg-muted/50')}>
                ...
                <div className="mt-1 text-[11px] text-muted-foreground">{b.focusTag.replaceAll('_',' ')} · {b.mode}</div>
                <Progress value={pct} className="mt-3 h-1.5"/>
                <div className="mt-1.5 flex justify-between text-[10px] text-muted-foreground"><span>{pct}% mastery</span><span>Lv {b.difficultyCeiling}</span></div>
              </button>
            })}
            <Separator/>
            <div className="rounded-xl bg-muted/50 p-3"><div className="flex items-center gap-2 text-xs font-semibold"><Brain className="h-3.5 w-3.5 text-primary"/>Due repairs</div><div className="mt-1 text-2xl font-bold">{dueCount}</div><p className="text-[11px] text-muted-foreground">Recurring mistakes waiting for another rep.</p></div>
          </CardContent>
        </Card>
        ...
        <Card className="h-fit overflow-hidden">
          ...
          <CardContent className="space-y-4 pt-0">
            {overview.dueErrors.length>0 && <div className="rounded-2xl border border-amber-400/30 bg-amber-50/50 p-3 dark:bg-amber-950/15"><div className="flex items-center gap-2 text-xs font-semibold"><RotateCcw className="h-3.5 w-3.5 text-amber-500"/>Recent mistakes</div><div className="mt-2 space-y-2">{overview.dueErrors.slice(0,4).map(e=><div key={e.id} className="rounded-xl bg-background/80 p-2.5 text-xs"><span className="font-medium line-through decoration-destructive/60">{e.wrong}</span><span className="mx-1 text-muted-foreground">→</span><span className="font-semibold text-emerald-600 dark:text-emerald-400">{e.right}</span><div className="mt-1 text-[10px] text-muted-foreground">{e.errorType}</div></div>)}</div></div>}
            <div><div className="mb-2 text-xs font-semibold text-muted-foreground">Weakest skills</div><div className="space-y-2">{overview.mastery.slice(0,6).map(m=><div key={m.tag}><div className="mb-1 flex justify-between text-[11px]"><span>{m.tag.replaceAll('_',' ')}</span><span>{Math.round(m.mastery*100)}%</span></div><Progress value={m.mastery*100} className="h-1.5"/></div>)}</div></div>
            <div className="rounded-2xl bg-primary/5 p-3"><div className="text-xs font-semibold">How the Mentor works</div><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">It asks before explaining, stays inside your focus, diagnoses the root cause, and schedules your mistakes for repair.</p></div>
          </CardContent>
        </Card>
      </div>
